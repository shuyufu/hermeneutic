...
從多個交易所（目前做 Binance USDⓈ-M Futures + Binance Spot + Bybit linear/spot）訂閱多個 symbol 的 L2 order book diff，維護正確的本地 order book 狀態，餵進已經存在的 `SymbolBook`（`apps/aggregator/aggregator_service.hpp`，`AggregatorService::book(symbol)`）。目標是非同步、多執行緒（Boost.Asio + Beast），且核心邏輯採 **sans-io** 設計：resync/序號校驗這類最容易出錯的邏輯完全不碰 socket，可以純用假資料序列做單元測試。
...
## 3. `SymbolBook`（已實作：`apps/aggregator/aggregator_service.hpp`）
...
`.proto` 的 `SubscribeRequest` 加了 `string symbol = 1`（安全,因為這個 proto 從沒服務過真正的 consumer）。

**現況（本節其餘部分是原始設計時的描述，已被後續改動取代，保留不動作為歷史記錄）**：`books_` 已從 `map<string, SymbolBook, less<>>` 換成以結構化 `BookId` 為 key 的 `unordered_map`，`SubscribeRequest{symbol}` 也已換成帶 `BookId` 的 `SubscribeBboRequest`/`SubscribeL2DiffRequest`——見第 10 節第 10 項的完整過程與理由。

## 4. `SymbolSync<SequencePolicy>`（已實作：`include/bobby/hermeneutic/book/symbol_sync.hpp` / `tests/book/symbol_sync_test.cpp`）
...
寫在框架設計階段的每一個 `SymbolSync`/`SymbolSyncBybitTest` 測試都是照 Binance 的 REST-race 順序（`on_depth_update()` 先於 `on_snapshot()`）驅動事件，沒有一個真的模擬過 Bybit 的實際訊息順序，所以完全沒抓到——是接上真實 `server_main.cpp` 之後由一次程式碼審查（advisor）用手動追蹤程式碼流程抓出來的，不是任何自動化測試先發現的。修法是幫 `SequencePolicy` 加一個新的必要成員 `kTrustsConnectionOrder`（`BinanceFuturesSequencePolicy` 設 `false`，行為不變；`BybitSequencePolicy` 設 `true`），`on_snapshot()` 在「沒有 bridge」分支裡多一個 `if constexpr` 短路：`kTrustsConnectionOrder == true` 且**從一開始就沒有任何事件被 buffer 過**（不是「buffer 現在剛好是空的」——這兩者不同，見下一段）時，直接把 snapshot 當成起點進入 Live 狀態，不再要求先有一個 bridge 事件。
...
2. 上面這個修法本身還有第二個漏洞：live 狀態下的 gap 分支（`on_depth_update` 裡 `is_contiguous` 失敗那條路）也會 `reset()` 後把觸發 gap 的事件塞回 `buffer_`——這是 `buffer_` 的第二個寫入點，不只 Buffering 狀態那一個。如果誤判邏輯只在意「進入函式當下 `buffer_` 是否空」，這個路徑塞進去的事件一樣會在下一次 `on_snapshot()` 被正確納入判斷（因為那時候 `buffer_` 真的非空），所以捕捉時機本身（函式入口）已經同時涵蓋兩個寫入點，不需要額外用一個獨立的 bool flag 去追蹤「Buffering 分支有沒有真的塞過東西」——若真的改用一個只在 Buffering 分支裡設定的 flag，反而會漏掉這條 live-gap 路徑，重新踩進同一個坑。三個對應的判別測試：`SnapshotArrivingBeforeAnyBufferedEventGoesLiveDirectly`（驗證修好的那個真實 bug）、`NonEmptyBufferThatDoesNotBridgeStillRetriesDespiteTrustingConnectionOrder`（驗證捷徑不會亂吃真正的 gap）、`PostGapBufferedEventIsNotDiscardedByTheEmptyBufferShortcut`（驗證第二個寫入點沒被漏掉）——都在 `tests/book/symbol_sync_test.cpp`。

**當時尚未解決、列在第 10 節第 8 項的相關限制，現已修好**：live 狀態下發生 gap 時，`on_depth_update()` 只回傳 `InvalidateVenue`，並不會主動要求重新連線或重新拿 snapshot這件事本身——`SymbolSync` 完全沒變——但 `VenueSession` 現在會在看到這個 `InvalidateVenue`（且 `kTrustsConnectionOrder` 為真）時主動觸發整條連線重連，不再乾等。細節見第 10 節第 8 項。

**另一個真的擋掉資料流的 bug，這次在 bridge 之後的緩衝尾端重播邏輯**：`on_snapshot()` 找到 `bridge` 後，會把從它開始到 `buffer_.end()` 的每一筆緩衝事件依序 `ApplyDelta`——但這個重播迴圈原本只更新 `last_final_id_`，從來沒有拿相鄰兩筆之間的關係去跑 `SequencePolicy::is_contiguous()`。`bridge` 本身只保證跟**snapshot**銜接（`bridges_snapshot()`），不保證跟它後面那一筆緩衝事件之間沒有斷檔——兩筆都通過 `should_drop_buffered()` 的數值篩選，彼此之間仍可能真的漏了一筆（例如 `VenueSession` 讀取迴圈解析失敗直接跳過的那則訊息）。原本的程式碼會靜默套用過這個斷檔，`last_final_id_` 被設成斷檔之後那筆事件的 `final_id`，後續 `on_depth_update()` 的 `is_contiguous()` 檢查完全看不出帳本已經漏資料——直到下一次真正斷線重連整本被 snapshot 蓋掉之前，帳本會一直帶著這個錯誤跑。是由另一個 session 做全專案架構複查、逐行核對程式碼流程後回報的，不是任何自動化測試先發現的：既有測試裡唯一涵蓋「bridge 後有多筆緩衝事件」的案例（`BridgingSnapshotAppliesSnapshotThenBufferedTailInOrder`）刻意把尾端事件寫成彼此連續，從沒測過尾端本身有斷檔的情況。修法：重播迴圈裡，`bridge` 之後的每一筆都先跑 `is_contiguous()`（用剛更新過的 `last_final_id_`），不連續就中斷重播、把這筆（含）之後的事件重新收進 `buffer_`、`InvalidateVenue`、回到 Buffering——邏輯上直接複用 `on_depth_update()` 在 Live 狀態下遇到斷檔時的同一套處理。回歸測試：`tests/book/symbol_sync_test.cpp` 的 `BridgingSnapshotDetectsAGapBetweenTwoBufferedTailEvents`（兩筆緩衝事件之間人為留一個斷檔，驗證第二筆被判定成 gap 而不是被靜默套用；拿掉修法重跑會失敗，不是通過）。
...
## 6. `VenueFeed`（已實作：`include/bobby/hermeneutic/exchange/binance/binance_futures_feed.hpp` / `tests/exchange/binance/binance_futures_feed_test.cpp`）
...
### `BybitLinearFeed` / `BybitSpotFeed`（已實作：兩者現在同在 `include/bobby/hermeneutic/exchange/bybit/bybit_feed.hpp`，共用解析邏輯在 `include/bobby/hermeneutic/exchange/bybit/bybit_wire.hpp`）
...
- **即時驗證**：linear 跟 spot 各自獨立驗證，過程中發現並修正了 `SymbolSync` 的真實 bug（見上一節），所以驗證分兩輪：
  ...
  - 三個 venue 同時掛著（`server_main.cpp` 實際部署的樣子）沒有另外再做一次端對端驗證——三條路徑各自都已經單獨驗證過，`SymbolBook::apply_batch`/`invalidate_venue` 的 per-`VenueId` 隔離也已經在第 10 節第 2 項驗證過，沒有理由三個一起跑會表現不同，但這是一個尚未實測的假設，不是已驗證的事實，記在這裡。
...
- **`WebSocketConnection`（已實作：`include/bobby/hermeneutic/net/websocket_connection.hpp` / `tests/net/websocket_connection_test.cpp`）**：transport 層，venue-agnostic，薄。一條 WS 連線的生命週期本身（connect/send/read/close），包 `beast::websocket::stream<NextLayer>`。完全不知道 symbol/resync/交易所是什麼。**每次重連 = 新的一個 instance**，`VenueSession` 自己不會斷。
...
- **HTTP snapshot（已實作：`include/bobby/hermeneutic/net/http_client.hpp` / `tests/net/http_client_test.cpp`）**：不需要獨立的「HttpConnection」物件，寫成一次性函式 `http_get<NextLayer>(host, port, target, stream_args...) -> awaitable<expected<string, errc>>` 就夠，因為 REST snapshot 只是偶發的一次性 GET，不是常駐連線。跟 `WebSocketConnection` 同樣的 template 手法（`NextLayer` 決定要不要走 SSL），也跟它共用同一個 `detail::is_ssl_stream_v` trait（抽到 `include/bobby/hermeneutic/net/net_traits.hpp`，避免兩邊各自重複定義）。測試涵蓋成功回應、非 200 狀態碼、連線被拒絕三種情況，一樣用本地 plain TCP HTTP server，不用測試憑證。
- **`VenueSession<Feed, Policy, NextLayer>`（已實作：`include/bobby/hermeneutic/ingestion/venue_session.hpp` / `tests/ingestion/venue_session_test.cpp`）**：真正的 orchestrator，生命週期橫跨很多次 `WebSocketConnection`。擁有這條 session 負責的所有 symbol 的 `SymbolSync<Policy>`（`unordered_map<NativeSymbol, SymbolSync<Policy>>`），驅動迴圈：
...
## 8. `SymbolRegistry`（已實作：`include/bobby/hermeneutic/ingestion/venue_session.hpp`）
...
**現況速覽**（每個項目自己的小節仍是完整的過程記錄，這裡只是幫忙快速定位，不是取代下面的內文）：

- **已完成**：1（`apply_batch`）、2（`IngestionRunner`/`IVenueSession`）、5（Binance Spot `SequencePolicy`）、6（正式環境 SSL context）、7（`server_main.cpp` 接上六個 venue）、8（`kTrustsConnectionOrder` venue 的強制重連）、9（加入 OKX）、10（`BookId`/`VenueId` 結構化，含後續 `Exchange` 改名等連帶修正）、11～14（REST venue 在各種 gap/拒絕情境下卡死的四個發生點）、15（`live_venues` heartbeat + WS idle-read 偵測，已合併到 `main`，見下方狀態列更新）。
- **尚未解決 / 待做**：
  - 3：backoff/jitter 的實際數值未經真實流量調校（形狀已定）。
  - 4：多執行緒 `io_context` thread pool 大小策略未定（strand 已就位，見第 2 項）。
  - 第 8/9 項提到的已知限制（非 bug,刻意接受）：`kTrustsConnectionOrder` venue（Bybit/OKX）mid-stream 的 gap 仍會讓整條連線斷線重連,不是原地重試。
  - 第 13 項附帶記錄的問題：REST venue 的 snapshot fetch 沒有 per-symbol 去重/取消機制,重連疊加 gap 時有機會對同一個端點連續發送、觸發對方 rate limit——已記錄方向,未實作。
  - 第 15 項附帶記錄的已知盲點：LB/reverse proxy 獨立於 backend 處理 WS ping/pong 時,transport 層探測看不穿這一層；對應的長門檻「資料新鮮度」backstop 構想也還沒實作。


1. ~~`SymbolBook::apply_batch`~~ **已完成**：一次呼叫套用整批 bid/ask 變化，只 bump 一次 `seq_`、只 broadcast 一次，跟 `apply_snapshot` 一樣的「先驗證全部 level、再套用」模式（負數 size 任一 level 有錯，整批都不套用）。診斷結果的 bids/asks 順序沿用 `capture_snapshot_before` 的做法（用跟 aggregate map 相同的 comparator 建中繼 map），維持「diff 順序跟 snapshot 順序一致」這條既有的協定保證，不是照輸入順序原樣印出。`VenueSession::execute_action` 處理 `ApplyDelta` 這個 action 時已經改成呼叫這個新方法（`include/bobby/hermeneutic/ingestion/venue_session.hpp`），不再對 bids/asks 各自逐筆呼叫 `apply_delta`。新增 3 個測試（一次 seq bump、跨 venue 正確加總、負數 size 整批拒絕且不 broadcast）。
2. ~~`IngestionRunner`/`IVenueSession` 的 type-erasure 邊界~~ **已完成**：`include/bobby/hermeneutic/ingestion/ingestion_runner.hpp`（`IVenueSession`、`VenueSessionAdapter<Feed,Policy,NextLayer>`、`IngestionRunner`）+ `VenueSession` 本身加上 strand/cancellation/`stop()`（`include/bobby/hermeneutic/ingestion/venue_session.hpp`）。測試：`tests/ingestion/venue_session_test.cpp` 的 `StopAbortsBackoffWaitAndDoesNotReconnect`/`StopDrainsInFlightSnapshotFetch`，`tests/ingestion/ingestion_runner_test.cpp` 的 `StopAllLetsIoContextFinishWithoutIoStop`。`server_main.cpp` 已改用 `IngestionRunner`。細節與過程中修正記錄如下。
   ...
   **`stop()` 是這一項真正要解決的問題，不是裝飾**。`run()` 原本是 `while(true)` 迴圈、完全沒有取消機制；`server_main.cpp` 原本靠 `io.stop()` 把整個 `io_context` 連同所有 pending handler 一起粗暴丟掉，不是優雅關閉。做到「呼叫 `stop()` 之後乾淨結束、不留 in-flight 的 HTTP fetch/WS 讀取」，靠的是下面四件事，缺一個都不算數：
   ...
   **Code review 第二輪抓到、已修好的兩點**：
   ...
   - **`server_main.cpp` 拿掉 `io.stop()` 之後，收尾沒有 bounded-time 的兜底**：原本粗暴的 `io.stop()` 順便也是「不管三七二十一，時間到了就返回」的保證；改成純 `runner.stop_all()` 之後，如果 `drain_pending_snapshots()` 真的卡住不收斂（例如某個未來場景下 cancellation 沒能讓某個 async 操作真的中止），`io_thread.join()` 會無限期卡住，正式環境的 process 就真的關不掉。修法：仿照 `IngestionRunnerTest` 裡的 3 秒 watchdog 手法，在 `server_main.cpp` 加一個 `kShutdownTimeout`（10 秒）的 watchdog 執行緒——正常情況下（已經實測，見上面「已對真實 Binance Futures 實測」，`join()` 3ms 內就返回）watchdog 完全不會被觸發；真的卡住的話，才強制 `io.stop()` 兜底，並且印一行 log 說明發生了什麼，不再是無聲卡死。
   ...
   **Code review 第二輪提過、確認過但暫不處理的幾點**：
   ...
   - `IngestionRunner::stop_all()` 本身不回報「全部真的收尾完了」——「`on_done` 觸發後才能安全銷毀 session」這個承諾，目前是靠唯一的呼叫端（`server_main.cpp` 在 `stop_all()` 後面接著 `io_thread.join()`）湊巧做對，`IngestionRunner`/`IVenueSession` 本身沒有把這個承諾做成 API 的一部分。之後如果出現第二種呼叫端（例如不是靠 join 一個專屬 `io_context` 的方式），需要重新設計一個「等全部 session 真的 drain 完」的介面，不是現在就加。
...
5. ~~Binance Spot 的 `SequencePolicy`~~ **已完成**：`BinanceSpotSequencePolicy`（`include/bobby/hermeneutic/exchange/binance/binance_spot_sequence_policy.hpp`）+ `BinanceSpotFeed`（`include/bobby/hermeneutic/exchange/binance/binance_spot_feed.hpp`），對照 developers.binance.com 的 Spot 文件直接 WebFetch 逐字核對（2026-09-18，見文末 Sources），不是套用 Futures 的公式：
   ...
   **`HttpRequestSpec` 跟 `detail::parse_decimal_string`/`parse_level`/`parse_levels` 抽到新的 `service/binance_wire.hpp`**，原本只在 `binance_futures_feed.hpp` 裡私有定義——`VenueSession` 本來就是靠 `auto spec = feed.snapshot_request(symbol)` 鴨子定型讀取 `.host`/`.port`/`.target`（已確認 grep 過 `venue_session.hpp` 完全不 include 任何 Feed header、不具名引用 `HttpRequestSpec` 型別），理論上兩個 Feed 各自定義一份同名 struct 也不會被 `VenueSession` 擋下來，但 `server_main.cpp` 一份 `.cpp` 同時 include 兩個 Feed header 時，兩份完全相同的 `namespace bobby::hermeneutic::ingestion { struct HttpRequestSpec {...}; namespace detail { ... } }` 會直接 ODR 違規（重複定義）。這是真正的兩個使用者才動手抽的（不是預先設計），`binance_futures_feed.hpp` 也已經改成 include 這個共用 header，不再重複定義。
   ...
   **`server_main.cpp` 已接上兩個 venue**：每個 symbol 同時建 `IngestionRunner::add<BinanceFuturesFeed, BinanceFuturesSequencePolicy, ...>` 跟 `add<BinanceSpotFeed, BinanceSpotSequencePolicy, ...>`，各自一份獨立的 `SymbolRegistry`（不能共用同一份再 `std::move` 兩次——第一次 `add()` 會把 registry 整個搬空，第二個 venue 拿到的會是 moved-from 的空 map，`registry_.book(symbol)` 永遠回傳 `nullptr`）。~~同一個 `SymbolBook` 因此會同時收到 spot 跟 perp 兩個 `VenueId` 的貢獻，聚合後的 aggregate book 是兩個市場流動性的合併~~ **這個「spot+perp 合併進同一本 book」的決定，第 9 節加 OKX 的時候被推翻了**：spot 跟 perp 是不同的商品、不同的價格跟流動性，合併從來就不是刻意設計的結果，只是碰巧每個 venue 都拿裸的 canonical symbol 當 registry key。現在 Futures/Spot 各自接 `.PERP`/`.SPOT` 兩個獨立的 `SymbolBook`，見第 9 節。
6. ~~正式環境的 `net::ssl::context` 建構/憑證驗證設定~~ **已完成並實測**：`server_main.cpp` 接上 ingestion 後第一次真的編譯到 `net::ssl::stream<beast::tcp_stream>` 這個 template 實例化，發現漏了 `#include <boost/beast/websocket/ssl.hpp>`（Beast 對 SSL stream 的 `async_teardown` customization point 是獨立 header，沒 include 的話會在 `boost/beast/websocket/teardown.hpp` 出現 `static_assert(sizeof(Socket)==-1, "Unknown Socket type in async_teardown.")`）。修好後**實際跑起來連上 `wss://fstream.binance.com/ws` + `https://fapi.binance.com`，收到真實 BTCUSDT order book**（snapshot 1928 bids/1854 asks，diff 持續進來）。另外完成一次約 19 分鐘的即時雙 symbol（BTCUSDT、ETHUSDT，同一個 `hermeneutic_aggregator_service` process）穩定性驗證：全程只建立 1 次連線（無斷線重連），雙邊都收到真實 snapshot 並持續套用 diff（各自 diff_count 達 7500+ 才手動停止，不是自然結束），heartbeat 全程以預期節奏送達，驗證用的 log 裡沒有任何 error/disconnect 紀錄。（負責跑這個驗證的背景 agent 自己中途就停在一則「等 15 分鐘再回報」的訊息、沒有真的送出最終報告或清掉留下的兩個 process；這份紀錄是事後直接讀它留下的 log、確認狀態正常後，手動收尾補上的。）
7. ~~`server_main.cpp` 尚未接上 ingestion~~ **已完成，現在接六個 venue**：見 `apps/aggregator/server_main.cpp`——建構 `AggregatorService` 後，額外建一個 `net::ssl::context`（`tlsv12_client`，`set_default_verify_paths()` + `verify_peer`），透過 `IngestionRunner::add<Feed,Policy,NextLayer>()` 接上 `BinanceFuturesFeed`/`BinanceFuturesSequencePolicy`（VenueId `"binance_futures"`）、`BinanceSpotFeed`/`BinanceSpotSequencePolicy`（VenueId `"binance_spot"`）、`BybitLinearFeed`/`BybitSequencePolicy`（VenueId `"bybit_linear"`）、`BybitSpotFeed`/`BybitSequencePolicy`（VenueId `"bybit_spot"`）、`OkxFeed`/`OkxSequencePolicy`（VenueId `"okx_spot"`/`"okx_swap"`，同一個 Feed/Policy 型別接兩次，見第 9 節）六個 venue，`co_spawn` 上一個獨立的 `io_context`（自己的 thread 跑 `io.run()`），跟原本的 gRPC server／heartbeat thread 並存，`server->Wait()` 回來後 `runner.stop_all()` + join 收尾。**每個 venue 一份獨立的 `SymbolRegistry`，不是共用一份再 `std::move` 好幾次**：`VenueSession` 的建構子把 `registry` 整個按值搬進去，同一份 registry 傳給後面的 `add<>()` 只會拿到 moved-from 的空 map，`registry_.book(symbol)` 永遠回 `nullptr`。~~同一個 `SymbolBook` 因此同時收四個 venue 的貢獻，聚合後是四邊流動性的合併~~ **spot/perp 從第 9 節起分成兩本獨立的 `SymbolBook`**：perp/futures/swap 三個 venue（Binance Futures、Bybit linear、OKX swap）寫進 `.PERP` 那本，spot 三個 venue（Binance Spot、Bybit spot、OKX spot）寫進 `.SPOT` 那本，`SymbolBook::apply_batch`/`invalidate_venue` 本來就是 per-`VenueId`（見第 10 節第 2 項）這件事現在用在「同一本 book 裡三個 venue 各自記帳」上，不是「六個 venue 全部混在一本 book 裡」。之後真要再加第七個交易所，一樣走 `IngestionRunner::add<>()`，不需要再動這層邏輯。
8. ~~`SymbolSync` 的 live-gap 恢復機制，對 `kTrustsConnectionOrder == true` 的 venue（目前是 Bybit/OKX）比對 Binance 更需要處理~~ **已完成**：修法就是當初列在這裡的方向——`VenueSession`（`include/bobby/hermeneutic/ingestion/venue_session.hpp`）新增 `execute_actions_and_maybe_force_reconnect()`，包住 `dispatch_depth_update()`/`dispatch_snapshot()`/`execute_action()` 裡 `apply_failed` 觸發的 resync 這三個呼叫點：先照常執行 actions，若 `if constexpr (Policy::kTrustsConnectionOrder)` 為真且 actions 裡出現過 `InvalidateVenue`，就 `throw` 一個 `std::runtime_error`，藉此重用 `run()` 既有的例外處理路徑（`catch` → `invalidate_all()` → backoff → 重新 `connect()`）把整條連線斷線重連——不是新增一個獨立的 `SyncAction`：`kTrustsConnectionOrder` 這條路徑本來就假設一條連線只服務一組有序的 snapshot-then-diff 串流，重連本來就是這個 policy 唯一的 resync 手段，跟真正斷線時的處理完全同一套。`SymbolSync` 本身完全沒改動這部分邏輯（`on_depth_update()`/`on_disconnected()` 的回傳值不變），純粹是 driver 層多讀一次 actions 內容再決定要不要多做一件事。回歸測試：`tests/ingestion/venue_session_test.cpp` 的 `LiveGapForcesFullReconnectForTrustConnectionOrderVenue`（假 WS server 故意不主動關閉第一條連線，只有 `VenueSession` 自己觸發重連才會看到第二條連線送來的新 snapshot；拿掉修法重跑這個測試會在等不到第二條連線的情況下逾時失敗，不是掛住不動）。Binance（`kTrustsConnectionOrder == false`）維持原本的限制不動——它的 `RequestSnapshot` 真的會打 REST，之後若要修可以走「原地重新 fetch」而不必整條重連，跟這裡的修法不是同一個方向，留給以後。

    **同一類 bug 的第三個發生點，第三輪 `/code-review` 才抓到**：上面這個修法本身沒問題，但 `on_snapshot()` 還有另一個「沒找到 bridge」的分支——buffer 在進入函式當下**非空**、但裡面沒有任何事件跟這個 snapshot 銜接（`kTrustsConnectionOrder` 的「進入當下 buffer 完全沒被寫過」捷徑不適用時，落到的那條路徑）——原本只回傳 `RequestSnapshot`，從來不會回傳 `InvalidateVenue`。對 `kTrustsConnectionOrder` venue 來說這跟前兩個已修好的路徑是同一個死結：`RequestSnapshot` 對這類 venue 是 no-op，`VenueSession` 的 `execute_actions_and_maybe_force_reconnect()` 只在 actions 裡看到 `InvalidateVenue` 才會強制重連，這條路徑永遠不會觸發——症狀完全一樣（永遠卡在 `Buffering`、安靜地不貢獻資料），只是進入點不同（還沒進入 Live 狀態，在 Buffering 階段就卡死，不是 Live 狀態下發生 gap 才卡死）。修法：這個分支對 `kTrustsConnectionOrder` 為真、且非「從未 buffer 過」的情況，改呼叫 `handle_gap(std::move(buffer_))`，跟另外兩個路徑共用同一套「invalidate + reset + 保留未解決事件」邏輯。`VenueSession` 那一層完全不用改——`execute_actions_and_maybe_force_reconnect()` 早就會掃描 actions 裡有沒有 `InvalidateVenue`，不管是從 `on_snapshot()` 的哪個分支冒出來的。連帶把 8 個既有測試（`SymbolSyncBybitTest`/`SymbolSyncOkxTest` 各 4 個，凡是「on_depth_update() 先於 on_snapshot()、snapshot 沒銜接上」這個形狀的）的期望值從 `RequestSnapshot` 改成 `InvalidateVenue`，並新增一個 `VenueSession` 層級的回歸測試 `BufferingStateGapForcesFullReconnectForTrustConnectionOrderVenue`（同樣是「假 server 故意不關第一條連線，只有真的強制重連才會看到第二條連線」的驗證手法；這個情境下 `InvalidateVenue` 本身因為這個 venue 從沒真的貢獻過任何資料，`invalidate_venue()` 是 no-op、不會廣播，所以測試能觀察到的唯一訊號就是重連後的新 snapshot）。
9. **加入 OKX（`include/bobby/hermeneutic/exchange/okx/okx_feed.hpp`，`OkxSequencePolicy`）**：Channel 選 `books`（400 檔增量深度 + WS 推 snapshot），不用 `books5`（每次全量快照，非增量）或 `books-l2-tbt`（VIP 限定）。
...
10. ~~`symbol.hpp::book_key()` 的字串格式是這個專案目前唯一還在製造「無法還原」歧義的地方，尚未決定要不要改，牽涉 breaking change~~ **已完成**：`native_symbol()`/`venue_id()` 都吃 `(Venue, BaseQuote, BookType)`、對整個 domain model保持 total；只有 `book_key()` 把 `{BTC, USDT}` 攤平成無底線的 `BTCUSDT.SPOT`/`BTCUSDT.PERP`，base/quote 的邊界從此永久消失——跟 `split_base_quote()` 自己註解講的「串接字串在沒有維護 quote-asset 字典時無法反推邊界」是同一個問題，也是這個專案先前刪掉 `okx_canonical()`（第 9 節、`5d5dd69`）的同一個理由：猜邊界是死路，`book_key()` 是最後一個還在做這件事的地方。判斷式：能不能寫出 `parse_book_key(s) -> optional<pair<BaseQuote, BookType>>` 當精確反函式？現在不行。連帶地，`book_subscription.hpp` 的 `seen_book_keys` 用串接後的字串去重，「不同的 (base, quote, type) 不會撞成同一個 key」這件事目前是靠實際幣別代碼的巧合成立，不是靠結構保證。
    ...
    **修正第10節第10項「延展性」小節的過度承諾,2026-09-19,同一天**：上面（本節第10項）那句「`symbol.hpp` 裡每個 exhaustive switch（`native_symbol()`/`venue_id()` 那種...）會編譯失敗直到補上處理」對 `MarketType` 這個維度其實不成立——使用者發現後查證：`to_string(BookId)`、`native_symbol()` 的 OKX 分支、`book_id.hpp` 的 `fill_wire_book_id()` 三處全部是二元 ternary（`id.type == MarketType::Spot ? X : Y`），不是 exhaustive switch；真正的 exhaustive switch 只存在於 `Exchange` 這個維度（`to_string(VenueId)`/`native_symbol()` 對 `Exchange` 的 switch、`exchange_name()`）。額外查證：這個專案的 `CMakeLists.txt` 完全沒設 `-Wall`/`-Werror`，但用 clang 直接測試確認 `-Wswitch`（enum switch 沒覆蓋完會警告）是 clang 預設就開的，不需要 `-Wall`——所以「編譯失敗」講法本身也偏誇大，實際是「會警告」，不是硬編譯錯誤。修法：把上述三處 ternary（連同一個原本沒查到、事後 grep 補到的 `to_string(VenueId)` 巢狀 ternary）全部換成真正的 exhaustive switch，讓 `MarketType` 也享有跟 `Exchange` 一樣的保護。同時查證後**決定不幫 `MarketType` 加 `Unspecified`/`Invalid` 成員**（proto 的 `MARKET_TYPE_UNSPECIFIED` 純粹是 proto3 未設定欄位的隱式零值，屬於 wire format 的需求，已經在 `book_id.hpp::to_symbol_book_id()` 這唯一的邊界處理掉，轉換成 `nullopt`；`symbol::MarketType` 本身沿用這個專案一貫的「無效狀態用 `optional`/`expected` 表示，不塞進 enum 本身」慣例，跟 `Asset`/`BaseQuote`/`BookId`/`Exchange` 全部一致，加了反而要在每個 switch 裡多處理一個「其實不該存在」的 case，且 `to_symbol_book_id()` 那個 switch 本來就得保留 `default:`（proto enum 在 wire 上是開放的），加了對稱性也沒有實際好處）。改動範圍：`symbol.hpp`（`MarketType` 定義處新增延展性/`Unspecified` 決策的說明註解、`to_string(BookId)`/`to_string(VenueId)`/`native_symbol()`）、`book_id.hpp`（`fill_wire_book_id()`）。純內部重構,行為不變,build+test 120/15/27/5/1 全過、無 `-Wswitch` 警告，單一 venue live 驗證 `to_string(BookId)`/`to_string(VenueId)`/`fill_wire_book_id()` 三條路徑全部正確。

    **`ParsedMessage` 去重 + `HttpRequestSpec` 判斷推翻重來,2026-09-19,同一天**：使用者發現 `okx_feed.hpp`/`binance_futures_feed.hpp`/`bybit_wire.hpp` 各自獨立宣告同一個 `using ParsedMessage = std::optional<std::variant<SnapshotMessage, DepthUpdate>>;`(`binance_spot_feed.hpp` 甚至靠 `#include` 整個 `binance_futures_feed.hpp` 來借用它)——跟第283行`parse_decimal_string`/`parse_level`/`parse_levels`當年同一種 ODR 風險。全部搬進 `service/json_wire.hpp`(此時還叫這個名字)。**同一輪使用者接著問 `binance_wire.hpp` 裡的 `HttpRequestSpec` 是不是也該搬——第一次判斷是「不用搬,OKX/Bybit 都是 `kSnapshotViaRest == false`,沒有第二個真實使用者」,這個判斷後來被使用者推翻**:真正該測的不是「現在有沒有第二個使用者」,而是「這個型別的內容本質上跟 Binance 有沒有關係」——查證 `venue_session.hpp` 實際呼叫路徑後發現 `HttpRequestSpec` 的三個欄位(host/port/target)跟 `net/http_client.hpp::http_get()` 自己的參數列一模一樣,是 `net` 層那個函式的參數包,跟 Binance 完全無關,只是恰好第一次抽出來時只有兩個 Binance feed 在用。改成搬進 `json_wire.hpp`,`binance_wire.hpp` 清空後直接刪除。**特別驗證過一個關鍵限制**:沒有把它搬進 `net/http_client.hpp` 本身(雖然那才是真正對應的函式)——因為 `http_get()` 需要 Boost.Asio/Beast/OpenSSL(`HERMENEUTIC_BUILD_SERVICE`,需要 vcpkg),而 `exchange/` 這一層(所有 Feed header)只需要 simdjson(`HERMENEUTIC_BUILD_INGESTION`,不需要 vcpkg)——搬進去會把每個 Feed header 拖進 vcpkg 依賴,破壞這個專案刻意維持的「不需要 vcpkg 就能建置/測試 Feed 解析邏輯」能力。用 `-DHERMENEUTIC_BUILD_SERVICE=OFF`(不帶 vcpkg toolchain file)重新設定+建置 `hermeneutic_binance_futures_feed_test`/`hermeneutic_binance_spot_feed_test` 實測驗證過這個限制依然成立。**檔案改名**:`json_wire.hpp` 這個名字對裝了非 JSON 內容(`HttpRequestSpec` 跟 JSON 完全無關)的檔案不再準確,改名成 `feed_wire.hpp`(所有 include 路徑跟兩處引用它的 comment 一併更新)。Build+test 120/15/27/5/1 加五個交易所 feed test(12/11/8/10/13)全過,單一 venue(Binance spot,會真的走 REST snapshot 路徑)live 驗證通過。

11. ~~`handle_request_snapshot()` 的 REST fetch 失敗時直接 `co_return`，靠「next reconnect or steady-state gap retries」補救——這個假設對 `kTrustsConnectionOrder == false` 的 venue（Binance）不成立~~ **已完成，2026-09-21**：由另一個 session 在討論 connection-event callback/book validity 降級設計時發現——第一次 REST snapshot 抓取就失敗的話，symbol 從沒進入過 Live，`on_depth_update()` 在 Buffering 狀態下只會單純 `push_back` 進 `buffer_`、完全不做任何連續性檢查（那個檢查只存在於 Live 分支），所以「steady-state gap retry」這條假設中的補救路徑根本走不到；剩下唯一能重新觸發 `RequestSnapshot` 的只有一次不相關的 WS 斷線意外重置 `snapshot_requested_`。修法：`handle_request_snapshot()` 改成一個迴圈，fetch 失敗就 `co_await backoff(attempt)`（跟 `run()` 自己重連用的同一個 `backoff()`）後原地重試，完全不碰 WS 連線、不呼叫 `should_force_reconnect()`/`run_sig_.emit()`——這點特別重要：`should_force_reconnect()` 對能走到這個分支的 venue（`kSnapshotViaRest == true` 在這個專案裡只會跟 `kTrustsConnectionOrder == false` 配對）永遠回傳 `false`，如果誤用強制重連機制，等於無條件繞過這個 policy gate，把「一個 symbol 的 REST GET 失敗一次」升級成「同一個 `VenueSession` 底下所有 symbol 一起斷線重連」——這正是第 10 節第 8 項一開始就講明要避免的方向（REST venue 該原地重試，不是拆連線）。回歸測試：`tests/ingestion/venue_session_test.cpp` 的 `FailedSnapshotFetchRetriesInPlaceWithoutTouchingTheConnection`（假 HTTP server 第一次連線直接關掉不回應、第二次才正常回應；WS 連線全程不斷；拿掉修法重跑這個測試會逾時失敗，不是掛住不動）。跟第 10 節第 8 項的三個修法是同一個「stuck forever」問題家族，但發生條件、觸發路徑、正確修法方向都相反（一個要強制重連、一個絕對不能強制重連），因此故意分開記錄，不要混在一起。

12. ~~`handle_gap()`（`SymbolSync`）對 `kTrustsConnectionOrder == false` 的 venue（Binance），Live 狀態下發生 gap 之後只回傳 `InvalidateVenue`，沒有任何機制再要求新的 snapshot~~ **已完成，2026-09-21**：由另一個 session 對合併後的最終版做逐行複查發現——跟第10節第11項是同一個「stuck forever」問題家族，但發生在不同的觸發路徑：第11項是「第一次 REST 抓取就失敗，symbol 從沒到過 Live」；這項是「已經到過 Live，中途發生 sequence gap」。`handle_gap()` 原本 reset 狀態回 `Buffering` 後只回傳 `InvalidateVenue`，而 `VenueSession::should_force_reconnect()` 對這類 venue 永遠回傳 `false`（刻意的——見第10節第8項，REST venue 不該因為一次 gap 就拆整條連線）；`on_connected()` 又只在真的重連後才會被呼叫——兩者疊起來，這個 symbol 會永久卡在 `Buffering`。**既有測試自己的假設已經暗示了這個缺口**：`tests/book/symbol_sync_test.cpp` 的 `LiveGapInvalidatesAndReBuffersTheTriggeringEvent`（Futures policy）原本在斷言完 `InvalidateVenue` 後，手動呼叫 `sync.on_connected()` 模擬「假裝重連發生了」才能拿到 `RequestSnapshot`——這只驗證了 `SymbolSync` 自己的狀態機邏輯，沒驗證真實系統裡 `VenueSession` 會不會真的觸發這次 `on_connected()`；答案是不會。修法：`handle_gap()` 內部依 `if constexpr (SequencePolicy::kTrustsConnectionOrder)` 分岔——`kTrustsConnectionOrder == true` 維持原樣（前一項修法已經靠 `InvalidateVenue` 觸發強制重連，不需要也不該再多一個 `RequestSnapshot`，那只是在連線被拆掉前多丟一個注定 no-op 的動作）；`kTrustsConnectionOrder == false` 則額外 `emplace_back(RequestSnapshot{})` 並把 `snapshot_requested_` 設成 `true`（比照 `on_connected()` 自己的「Buffering 且尚未請求過」邏輯），讓 `handle_request_snapshot()` 既有的（第11項剛加上、原地重試的）REST 抓取路徑接手，完全不需要碰 WS 連線。因為 `handle_gap()` 是三個發生點（Live-state gap、bridge-tail gap、buffer 非空但沒 bridge）共用的同一個 helper，這個修法自動涵蓋全部三個路徑，不需要逐一修——包括第4節那次 bridge-tail-gap 修法當時遺漏的同一個問題（`BridgingSnapshotDetectsAGapBetweenTwoBufferedTailEvents` 那個測試同樣是用手動呼叫 `on_connected()` 模擬重連，同一個假設瑕疵）。連帶更新這兩個既有測試：`gap_actions` 現在斷言 `{InvalidateVenue, RequestSnapshot}`，後面改成斷言「再呼叫一次 `on_connected()` 必須是空的（不能重複請求）」而不是「必須回傳 `RequestSnapshot`」。回歸測試：`tests/ingestion/venue_session_test.cpp` 新增 `LiveStateGapForARestVenueRetriesTheSnapshotFetchWithoutTouchingTheConnection`（WS 連線全程只建立一次、從不主動關閉；REST 服務兩次請求，第二次只有修法生效才會發生；拿掉修法重跑會逾時失敗）。四個修法都用「先拿掉修法確認測試失敗、加回去確認通過」驗證過，290 個測試全過。

13. ~~`execute_action()` 的 `apply_failed` 分支（書籍層級拒絕，跟真正斷線無關）透過 `SymbolSync::on_disconnected()` 處理 resync，對 `kTrustsConnectionOrder == false` 的 venue 沒有請求新 snapshot 的機制~~ **已完成，2026-09-21**：又一個 `/code-review` 逐行複查找到的同一問題家族第四個發生點——跟第11、12項的差別在於觸發來源不是 `SymbolSync` 自己偵測到的序號斷檔，而是**書本層級的資料驗證失敗**（`AggregateOrderBook::apply_snapshot()`/`apply_batch()` 拒絕畸形資料）。`on_disconnected()` 是刻意設計成「連線真的斷了，等真正重連的 `on_connected()` 才去抓 snapshot」（見它自己的註解：提早抓只會在連線恢復前就過期），但 `apply_failed` 這條路徑連線其實還活著，只是這筆資料本身有問題——沿用 `on_disconnected()` 就繼承了它「不主動請求」的語意，對 REST venue 等於再次卡死。修法：`SymbolSync` 新增 `on_book_rejection()`（`return handle_gap(buffer_.begin());`——呼叫這個方法時 `buffer_` 保證是空的，因為書本層級的拒絕發生在 `SymbolSync` 自己那批 actions 已經算完、內部狀態已經轉移之後），`execute_action()` 的 `apply_failed` 分支改呼叫它。連帶把 `handle_gap()` 裡 `kTrustsConnectionOrder == false` 那段從手寫「`snapshot_requested_ = true; actions.emplace_back(RequestSnapshot{});`」改成直接呼叫 `on_connected()`（此時 `state_`/`snapshot_requested_` 已經是它要檢查的前提，不用再抄一份規則——同一輪 review 的另一個發現：兩份規則以後可能各自漂移）。回歸測試：`tests/ingestion/venue_session_test.cpp` 新增 `BookRejectionRetriesTheSnapshotFetchForARestVenueWithoutTouchingTheConnection`——過程中意外抓到*自己*的一個計時問題（不是產品程式碼的 bug）：第二次 REST 回應如果比 gated WS 訊息（`run_fake_ws_server_two_messages_gated`，同一輪 review 對 `LiveStateGapForARestVenueRetriesTheSnapshotFetchWithoutTouchingTheConnection` 那個既有測試的計時缺口提出的修正手法，見下）送達 `SymbolSync` 還快，就會撞上「buffer 空、沒有 bridge、立刻再請求一次」這個既有（非本次改動造成）的迴圈，跟本項下面「同一輪 review 提出、目前沒有處理」那段描述的 hammering 機制是同一回事——加了一個可選的 `second_delay` 參數（`run_fake_http_server_twice`）解決，不是產品程式碼需要修的東西。三個修法（連同第11、12項）都用「先拿掉修法確認測試失敗、加回去確認通過」驗證過，291 個測試全過。

    **同一輪 review 額外找到、有修的一個測試品質問題**：`LiveStateGapForARestVenueRetriesTheSnapshotFetchWithoutTouchingTheConnection`（上面第12項新增的測試）原本用 `run_fake_ws_server_two_messages` 把兩則 DEPTH 訊息背靠背送出，沒有跟 REST 回應的到達時間同步——如果兩則訊息在第一次 snapshot 回應處理完之前就都已經進了 `buffer_`，第二則訊息之間的斷檔會被 `on_snapshot()` 的 bridge-tail-gap 分支（第4節那個修法）抓到，而不是這個測試名字宣稱要測的 `on_depth_update()` Live-state-gap 分支——斷言的 action/L2Update 序列剛好兩條路徑長得一樣，所以測試不會變 flaky、也不會失敗，只是「測到了，但測到的不是它宣稱要測的東西」，之後如果 Live-state-gap 分支單獨壞掉，這個測試也不會發現。修法：新增 `run_fake_ws_server_two_messages_gated`（多一個 `std::atomic<bool>* proceed` 參數，第二則訊息送出前先 poll 這個旗標），測試裡改成等到第一則訊息的 bridge（`wait_for(2)`）真的透過 gRPC 被讀取端的獨立執行緒觀察到之後才設旗標——不能從呼叫 `io.run_for()` 的主執行緒本身做這件事，因為那個呼叫本身就會整段時間佔住主執行緒。

    **同一輪 review 提出、目前沒有處理，只記錄的一個問題（跟上面「自己撞到的計時問題」是同一個機制，但情境更廣）**：`handle_gap()`（第12項）現在會在每一次 Live-state/bridge-tail gap 都嘗試發一次 REST snapshot 請求，不再只有連線剛建立時才會發生。如果一個不相關的 WS 斷線／重連剛好發生在某個 gap 觸發的 snapshot fetch 還在重試（`handle_request_snapshot()` 第11項加的原地 retry loop）的同時，`on_disconnected()`/`reset()` 會清掉 `snapshot_requested_`，讓重連的 `on_connected()` 為同一個 symbol 再發一個獨立的 fetch——兩個 fetch 之間沒有任何取消/去重機制（`snapshot_sigs_` 只有 `stop()` 會遍歷），後完成的那個可能對著一個已經在別處恢復 Live 的 `SymbolSync` 呼叫 `on_snapshot()`；如果那時候 buffer 剛好是空的，`kTrustsConnectionOrder == false` 的 no-bridge 分支會無條件立刻再 `RequestSnapshot`（`handle_request_snapshot()` 的 `backoff()` 只在 fetch 本身失敗時介入，這種「fetch 成功但沒 bridge」的情況完全沒有任何節流），有機會演變成短時間內對同一個 REST 端點連續發送、觸發對方 rate limit。這個「no-bridge 立刻無節流重試」的機制本身在改動前就存在（原本只在剛連線、buffer 還沒填的窄窗口內可能發生），第12項的修法只是讓它在整條連線的生命週期裡、任何一次 gap 之後都可能被觸發，範圍變廣了，不是這次改動生出的新機制。修法方向：VenueSession 層級對同一個 symbol 的 snapshot fetch 做去重/取消（收到新的 RequestSnapshot 時，如果同一個 symbol 已經有一個在飛的 fetch，先取消舊的），以及/或者對「fetch 成功但沒 bridge」的重試也套用 backoff，而不是只對「fetch 失敗」套用——需要比較完整的設計，先記錄，不在這次範圍內處理。

    **同一輪 review 也指出、當時尚未處理的一個架構層面觀察，現已完成（見第14項）**：`should_force_reconnect()` 的註解自稱是「要不要強制重連」這個判斷式唯一寫的地方，但 `handle_gap()`（`SymbolSync`，第12項）當時也獨立做了一次對稱的「要不要重新請求 snapshot」判斷——同一個 `kTrustsConnectionOrder` 決策拆成兩個檔案、兩層抽象各自處理一次。Review 認為這個拆分可能正是「第四個發生點（本項，`apply_failed` → `on_disconnected()`）當初會被漏掉」的部分原因。第14項把整個決策收回 `VenueSession` 一處，`on_book_rejection()`（上一段提到的那個方法）也隨之拿掉了——上一段的敘述保留不動，因為它精確記錄了當時的真實狀態，現況請看第14項。

14. ~~第13項自己指出的架構層面問題：REST venue gap 後該怎麼辦的決策拆成 `symbol_sync.hpp`（`handle_gap()`）跟 `venue_session.hpp`（`should_force_reconnect()`）兩處，外加測試檔案裡三份幾乎一樣的假 HTTP server accept/read/respond 邏輯~~ **已完成，2026-09-21**：對同一個 commit 又一輪 `/code-review` 提的兩個選配清理,使用者要求先處理。

    **決策集中化**：`SymbolSync::handle_gap()` 改回完全 venue-shape-agnostic——不再 `if constexpr (SequencePolicy::kTrustsConnectionOrder)`,永遠只是「`buffer_.erase(...); state_=Buffering; last_final_id_=0; snapshot_requested_=false; return {InvalidateVenue{}};`」,`on_book_rejection()`（第13項新增的方法）整個拿掉,`execute_action()` 的 `apply_failed` 分支改回呼叫 `on_disconnected()`（兩者現在本來就是同一件事：純粹 reset + 回報 `InvalidateVenue`,不摻雜任何「該怎麼辦」的判斷）。`VenueSession` 新增 `resync_rest_venue_after_gap()`——`kTrustsConnectionOrder == false` 時直接呼叫 `it->second.on_connected()`（此時 `state_`/`snapshot_requested_` 已經是它要檢查的前提)——`execute_actions_and_maybe_force_reconnect()`／`handle_request_snapshot()` 兩個既有的「decide what a gap means」呼叫點，`gap_detected` 為真時改成 `if constexpr (Policy::kTrustsConnectionOrder)` 走原本的強制重連（throw／`run_sig_.emit()`），否則呼叫這個新方法——四個觸發點（Live-state gap、bridge-tail gap、no-bridge gap、書本拒絕）現在全部收斂到這兩個呼叫點背後的同一份邏輯，不會再有「三個修好、一個漏掉」這種事。`SymbolSync` 那邊完全不需要知道 driver 層任何重連語意，維持原本「純狀態機、零 I/O」的設計原則。回歸驗證：暫時把 `execute_actions_and_maybe_force_reconnect()` 的 `else` 分支拿掉重跑,`LiveGapForcesFullReconnectForTrustConnectionOrderVenue`（`kTrustsConnectionOrder == true`）不受影響、`LiveStateGapForARestVenueRetriesTheSnapshotFetchWithoutTouchingTheConnection`（`false`）確實會失敗,證明這個集中化路徑是真的被走到,不是恰好殊途同歸。`tests/book/symbol_sync_test.cpp` 裡第11、12項改過的兩個測試（`LiveGapInvalidatesAndReBuffersTheTriggeringEvent`／`BridgingSnapshotDetectsAGapBetweenTwoBufferedTailEvents`）也跟著改回「`SymbolSync` 單獨測試時只回傳 `InvalidateVenue`,不會自己決定要不要重新請求 snapshot」，這個決策現在只在 `VenueSession` 層級的測試（`LiveStateGapForARestVenueRetriesTheSnapshotFetchWithoutTouchingTheConnection`／`BookRejectionRetriesTheSnapshotFetchForARestVenueWithoutTouchingTheConnection`）被驗證。

    **測試 helper 去重**：`tests/ingestion/venue_session_test.cpp` 的 `run_fake_http_server`／`run_fake_http_server_fails_once_then_succeeds`／`run_fake_http_server_twice` 三個假 HTTP server 各自把「接受連線、讀 request、（可能延遲）寫 response」這段 Beast 樣板程式碼重複寫了一次。抽出共用的 `respond_to_one_http_request(socket, body, delay = 0)`,三個 helper 現在只描述自己該接受幾個連線、用什麼順序——不再各自維護一份幾乎相同的 accept/read/respond 邏輯。

    291 個測試全過,跟第11-13項合併成同一個 commit（依既有偏好：review-driven follow-up 折進 feature commit,不另開新的）。

15. ~~下游 gRPC client 沒辦法分辨「這個 book 真的沒變化」跟「支撐它的每個 venue 都靜默死掉了」；另外 WebSocket 半開連線（TCP 還活著，venue 完全停止回應）完全偵測不到——`VenueSession::run()` 的讀取迴圈會永遠卡住，`InvalidateVenue`/backoff/reconnect 都不會被觸發~~ **已完成，2026-09-21/22，merged 到 `main`（`cbaff36`）**：兩個獨立缺口，在同一輪討論裡一起補。

    **`live_venues`（`Heartbeat` 新欄位）**：`repeated string`，per-symbol 記錄目前哪些 venue 還在貢獻這個 book（不是 `bool`——會抹掉「3 個 venue 掛 1 個」跟「唯一的 venue 掛了」的差異，這個區分正是設計的重點）。從 `AggregateOrderBook::venues()` 讀出，在 `SymbolBook::send_heartbeat()` 既有的 `mutex_` 臨界區內填值，避免跟 `apply_batch()`/`invalidate_venue()` race。搭既有的每秒 `Heartbeat` 送，不開新的 gRPC stream：全部 venue 一起斷線時完全沒有 `L2Diff`/`Bbo` 訊息可送，掛在資料訊息上的欄位在這個最壞情況下永遠送不出去。

    **WS idle-read 偵測，換了三輪才定案**：
    1. 第一輪想用 Boost.Beast 內建的 `websocket::stream_base::timeout`+`keep_alive_pings`，但拿一個「假 server 本身也是 `websocket::stream`、held 著一個 idle read」去測，完全不會觸發——當時誤判成「這個機制在這個環境下壞了」。
    2. 改成手刻一個 message-level 計時器（`VenueSession::read_with_idle_timeout()`，用 `net::steady_timer` 跟 `connection.read()` 賽跑），過程中踩到 `boost::asio::experimental::awaitable_operators::operator||` 的坑——它內部用的是 `wait_for_one_success()`，不是 `wait_for_one()`，斷線導致的**失敗**不算「贏」，逼著整個 race 傻等到 idle_timeout 跑滿，弄壞了既有測試 `ReconnectRepeatsSnapshotPushedAsFirstMessageFlow`（拖到 timer 跑完才回應，不是不回應）；改用 `net::experimental::make_parallel_group`+`wait_for_one()` 才是對的（後者「任一方先完成，不管成功失敗」的語意才是這裡真正需要的）。
    3. 真的用真實 server+client（不是拋棄式程式）連 Binance Futures 最安靜的 CTKUSDT perp 實測 5 分鐘，發現 message-level 做法有結構性的 false positive 問題：交易所完全沒有義務在市場安靜時推送任何東西（連空心跳都沒有），自然空窗就逼近 14 秒且持續往上探，逼近 30 秒的預設 idle_timeout。Binance 的協定層 ping（文件寫每 3 分鐘）幫不上忙，因為 Beast 的 `async_read()` 會把 ping/pong 完全在內部吃掉、不會讓 read 完成。回頭用一個真正的黑洞假 server（handshake 完就完全不再 read，不會自動回 pong）重測 Beast 內建機制——這次正確觸發 `beast::error::timeout`，證明第一輪的診斷是錯的：不是機制壞了，是當年的測試情境本身有問題（假 server 會自動回 pong）。換回 Beast 內建機制：`idle_timeout` 直接傳進 `WebSocketConnection::connect()`，`read_with_idle_timeout()` 整個刪掉，逾時直接以 `beast::error::timeout` 走既有的 catch/backoff/reconnect 路徑。

    曾考慮把 watchdog 綁在「book 是否真的更新」而非「連線有沒有回應」上，已否決：交易所一樣不保證安靜時會推送任何東西（連 book-level 的訊號都不例外），會撞上跟第 3 輪一樣的誤判問題，還會把「連線死了」（重連有用）跟「市場安靜」（重連沒用、純粹製造噪音）綁得更緊。

    **後續 `/code-review` 追加發現的已知盲點**：Beast 的 `keep_alive_pings` 證明的只是「終止這條 WS 連線的第一個 hop 有回應」，如果交易所架構前面有 LB/reverse proxy 且獨立於 backend 自己處理 WS ping/pong（邊界層終止 ping/pong 通常很便宜），backend 真的死了、LB 還活著的情況下這個機制會永遠回報「健康」——任何 transport 層級探測都看不穿這一層，`websocket_connection.hpp` 的 `connect()` 已加註解記錄，尚未解決；也順帶討論過一個對應的長門檻（10-30 分鐘量級）「資料新鮮度」backstop 構想（不觸發重連，只讓 `live_venues` 誠實反映「這個 venue 資料真的很久沒進來」），跟第 3 輪被否決的短門檻 book-watchdog 構想差別在於門檻大小跟用途、不是機制形狀——尚未實作。

    `idle_timeout` per-venue 可設定（JSON 訂閱設定檔 `idle_timeout_seconds`/`venue_idle_timeout_overrides`，預設 30 秒），主要當安全閥：不是因為個別 venue 市場安不安靜還重要（transport 層偵測不在乎這個），是因為每個 venue 的 WS gateway 對 ping 到底回應多快，還沒逐一 venue live 驗證過。

    **即時驗證**：全程用真正的 `hermeneutic_aggregator_service`/`hermeneutic_aggregator_client`（JSON config 驅動，不是拋棄式程式——先前寫過一次拋棄式驗證程式，使用者指出這個 repo 已經有能重用的真正 server/client，改用真的之後才連帶測到 JSON config 載入/驗證這條路徑本身）：OKX spot TRX-USDT（idle_timeout 壓到 1 秒，確認真實連線下沒有 false positive）、Binance Futures CTKUSDT perp（5 分鐘量出上面提到的 ~14 秒自然空窗數據）。真黑洞驗證本身是對本地假 TCP server 做的，不是對真實交易所——各 venue 的 WS gateway 對 protocol-level ping 到底回應多快，仍是前一段提到的待驗證項目。

    多輪 `/code-review` 折進同一個 commit（依既有慣例）：`parse_idle_timeout_config()` 對完全損毀的文件曾經誤標成「`idle_timeout_seconds` 格式錯誤」（自己新增的測試自己抓到）；`IdleTimeoutConfig` 解析補上完整測試（原本是 0）；`venue_idle_timeout_overrides` 指到沒訂閱的 venue 現在會啟動失敗而不是靜默無效；`idle_timeout_seconds`/override 的 `seconds` 補上上限驗證（`idle_timeout.hpp::kMaxIdleTimeout`，24 小時）——沒上限的話大到某個程度會在轉成 `websocket::stream_base::timeout::duration`（int64 奈秒解析度）時溢位，一個原本想設「很長」的值反而變成近乎 0，造成瘋狂斷線重連迴圈；override 逐筆解析的 catch-all 改成只吞 `NO_SUCH_FIELD`/`INCORRECT_TYPE`，其餘真正的文件損毀改成往外拋，不再被誤標成「這個 entry 的欄位名稱錯了」；`send_heartbeat()` 的 `venues()` 走訪去重複（改成走一次、兩個 heartbeat 共用）；`hermeneutic_aggregator_client`（這個 repo 唯一的第一方 gRPC client）原本完全沒有讀 `live_venues`，補上（只在變化時印，不是每秒都印）；`server_main.cpp` 兩個幾乎一樣的「config 指到沒接上的 venue，啟動失敗」區塊抽成共用 lambda。全部 330 個測試通過。

## 11. 下一步

`SymbolSync<SequencePolicy>` + `BinanceFuturesSequencePolicy`/`BinanceSpotSequencePolicy`/`BybitSequencePolicy`/`OkxSequencePolicy`（測試涵蓋四種 policy，包含共用的 `kTrustsConnectionOrder` 機制）、`BinanceFuturesFeed`（11 測試）、`BinanceSpotFeed`（12 測試）、`BybitLinearFeed`（10 測試）、`BybitSpotFeed`（8 測試）、`OkxFeed`（17 測試，含 `okx_canonical()`/`is_swap()`）、`WebSocketConnection`（1 測試）、`http_get`（3 測試）、`VenueSession<Feed,Policy,NextLayer>` + `SymbolRegistry`（整合測試）都已完成並測試通過。**`server_main.cpp` 已經接上 ingestion（第 10 節第 7 項），並且分別對 Binance Futures（`wss://fstream.binance.com`/`https://fapi.binance.com`）、Binance Spot（`wss://stream.binance.com:9443`/`https://api.binance.com`）、Bybit linear（`wss://stream.bybit.com/v5/public/linear`）、Bybit spot（`.../v5/public/spot`）各自單獨跑起來過，各自收到真實 BTCUSDT order book，Bybit 兩個 venue 也各自透過真正的 `SubscribeBbo` gRPC 路徑確認過能把資料送到訂閱者**（第 10 節第 6/7 項的 TLS/端對端驗證；六個 venue 同時掛著跑則還沒有另外驗證過，見第 6 節「即時驗證」小節最後一段的說明）。整條「真實交易所 WS/REST → resync → 套用進真實 book → 真實 gRPC 訂閱者收到正確結果」的路徑，Binance/Bybit 四個 venue 都不只是測試證明可以動，是真的連過真實交易所跑過一次——但 Bybit 這兩個 venue 第一輪的「驗證」其實是無效的（見第 6 節），因為當時 `SymbolSync` 有一個會讓 Bybit 安靜貢獻零筆資料的真實 bug（見第 5 節 `BybitSequencePolicy` 的修正記錄），第二輪單獨掛 Bybit venue 重跑才是真正證明有效的那次。**OKX 兩個 venue 也已經完成端對端驗證**：用跟 Binance Spot 那次同一招的臨時、用完即刪的 `service/live_okx_check_main.cpp`——真實 `AggregatorService`（book `"BTCUSDT.SPOT"`/`"BTCUSDT.PERP"`）+ 真實 gRPC server + 真實 `Aggregator::Stub` client 對兩個 symbol 各開一個 `SubscribeBbo` 串流 + 真實 `OkxFeed`/`OkxSequencePolicy` 的 `okx_spot`（`BTC-USDT`）/`okx_swap`（`BTC-USDT-SWAP`）兩個 `VenueSession` 連 `wss://ws.okx.com:8443/ws/v5/public`（2026-09-18）。跑 15 秒：兩個訂閱一開始都收到空書（`bid_present=0 ask_present=0`，`SubscribeBbo` 送出當下狀態的既有行為），snapshot 套用後很快兩邊都變成有值，之後持續收到真實更新——spot 55 筆 BBO、perp 119 筆（含收尾時 `stop_all()` 觸發 `invalidate_venue` 送出的最後一筆空書），全程無 crash、無錯誤 log，`kTrustsConnectionOrder` 的空 buffer 捷徑確實讓兩個 symbol 都直接從第一筆 snapshot 起步進 Live，不是理論上而已。
...
剩下的收尾項目（第 10 節）：`SymbolBook::apply_batch`（1，已完成）、`IngestionRunner`/`IVenueSession` 的多交易所 type-erasure 邊界（2，**已完成並測試**——`stop()` 的 cancellation 傳播/孤兒 snapshot 排空/strand 三件事是核心難點，見第 2 項內文）、backoff 參數調校（3，尚未解決）、多執行緒 `io_context` 策略（4，尚未解決）、Binance Spot 的 `SequencePolicy`（5，**已完成**，見上——Bybit linear/spot、OKX spot/swap 則是各自獨立的擴充，見第 6/7/9 節）、Bybit/OKX 的 live-gap 強制重連機制（8，**已完成**——mid-stream gap 仍會讓整條連線斷線重連而非原地重試，這是刻意接受的殘留限制，不是未解決的 bug，見第 8/9 項）。

**本節（下一步）寫於 `BookId`/`VenueId` 結構化重構（第 10 節第 10 項）跟 `live_venues` heartbeat/WS idle-read 偵測（第 10 節第 15 項，已合併到 `main`）之前，內容沒有更新**：上面幾段對 OKX/ASan/L2Diff 驗證的敘述仍然精確，只是不包含第 10 節第 10～15 項這些後來完成的工作。目前最新的完成狀態，請看第 10 節開頭的「現況速覽」。
