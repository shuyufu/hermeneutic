...
15. ~~下游 gRPC client 沒辦法分辨「這個 book 真的沒變化」跟「支撐它的每個 venue 都靜默死掉了」；另外 WebSocket 半開連線（TCP 還活著，venue 完全停止回應）完全偵測不到——`VenueSession::run()` 的讀取迴圈會永遠卡住，`InvalidateVenue`/backoff/reconnect 都不會被觸發~~ **已完成，2026-09-21/22，`worktree-venue-heartbeat-idle-timeout` 分支，尚未 merge 到 main**：兩個獨立缺口，在同一輪討論裡一起補。
