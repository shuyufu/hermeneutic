...
// double-constructor uses, and every rescaling operator in this file (and
// volume_bands.hpp's VWAP crossing math) shares. Requires `denominator` be
// strictly positive: the more obvious-looking "numerator +/- denominator/2,
// then divide" shape silently rounds the wrong way for a negative
// denominator (e.g. 15 / -10 = -1.5, which should round to -2, but that
// shape gives -1) -- callers with a possibly-signed denominator must
// normalize its sign themselves before calling this. `numerator`'s sign is
// unrestricted, so this stays usable for a future signed delta/adjustment,
// not just non-negative quantities.
